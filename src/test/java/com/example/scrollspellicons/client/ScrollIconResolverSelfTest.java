...
public final class ScrollIconResolverSelfTest {
    public static void main(String[] args) throws Exception {
        ...
        require(events, "LivingDeathEvent", "duel death interception"); require(events, "recordPlayerDeath(player)", "server death recording"); require(events, "event.setCanceled(true)", "vanilla death cancellation"); require(manager, "eliminatedPlayers", "authoritative eliminated-player state"); require(manager, "protectPendingRestore", "five-second return protection"); require(manager, "player.setInvulnerable(true)", "pending-player invulnerability"); require(manager, "player.deathTime = 0", "death animation reset"); require(manager, "serverTicks + 100", "absolute five-second restore deadline"); require(manager, "isInvulnerable()", "original invulnerability capture"); require(network, "broadcastEliminationSnapshot", "final spectator death snapshot"); require(network, "snapshotHealth", "zero-health snapshot normalization"); require(network, "? 0.0F", "eliminated health forced to zero");
