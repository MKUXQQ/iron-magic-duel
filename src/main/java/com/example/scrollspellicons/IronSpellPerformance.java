...
public final class IronSpellPerformance {
    ...
    public IronSpellPerformance(IEventBus modEventBus, ModContainer modContainer) {
        ...
        modEventBus.addListener(PerformanceConfig::onConfigLoading);
        // Both config lifecycle events implement IModBusEvent and must be
        // registered on the mod event bus. Registering Reloading on the
        // gameplay (NeoForge) bus crashes during startup before config load.
        modEventBus.addListener(PerformanceConfig::onConfigReloading);
