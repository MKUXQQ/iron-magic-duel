...
public final class ClientPerformanceEvents {
    ...
    public static void onFrame(RenderFrameEvent.Pre event) {
        if (!PerformanceConfig.isClientConfigLoaded()
                || !PerformanceConfig.clientValues().enableClientOptimizations()) {
            return;
        }
        ...
        particleBudget = new ParticleBudget(
                config.particleDistanceMultiplier(), config.maxParticlesPerFrame());
    ...
    private static void updateSpellProjectileTargets() {
        ...
        PerformanceConfig.ClientValues config = PerformanceConfig.clientValues();
        int maxVisible = config.maxVisibleSpellProjectiles();
        int renderDistance = config.projectileRenderDistance();
